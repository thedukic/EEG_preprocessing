...
%
%
% Edits by SDukic, July 2023
% Edits enable plotting on the same figure if an axis is given
%
...
if ~nargout || plotflag
 % Print result and display spectra
    ...
    % SDukic, Feb 2024
    % Var dpow added as an output
    dpow = nt_wpwr(x-y)/nt_wpwr(nt_demean(x));
    % disp('proportion of non-DC power removed:'); disp(nt_wpwr(x-y)/nt_wpwr(nt_demean(x)));
    fprintf('Proportion of non-DC power removed: %1.3e\n',dpow);

    % The ploting code is changed slightly
    % SDukic, July 2023
    %
    % Figure 1
    % figure(p.fig2); clf; subplot 121;
    axes(p.fig1);
    ...
    % Figure 2
    % subplot 122
    axes(p.fig2); % SDukic, July 2023
    x0 = sqrt(mean(x(:).^2));
    [pxx,f] = nt_spect_plot((x-y)./x0,p.nfft,[],[],1/fline);

    semilogy(f,abs(pxx)/divisor);
    hold on;
    [pxx,f] = nt_spect_plot(y./x0,p.nfft,[],[],1/fline);
    semilogy(f,abs(pxx)/divisor);
    ...
    set(hh(1),'linewidth',1.25);

    yl = [min(yl1(1),yl2(1)), max(yl1(2),yl2(2))];
    % subplot 121; ylim(yl); subplot 122; ylim(yl);
    axes(p.fig1); 
    ylim(yl); legend('original'); legend boxoff;

    axes(p.fig2); 
    ylim(yl); legend('noise','clean'); legend boxoff;
...
% Line-dominated components
xxxx = nt_mmat(xxxx,todss(:,1:nremove));
% Project them out
xxx  = nt_tsr(x-xx,xxxx);
clearvars xxxx;
