...
%
% If no output arguments, plots spectra of first DSS components for each bias
%
% nt_greetings;
...
if ~nargout || ~isempty(plotflag)
    ...
    set(gca,'ytick',1:K:numel(freqs), 'yticklabel',num2str(freqs(1:K:end), '%.3g'));
    ylabel('bias, Hz'); xlabel('frequency, Hz');
