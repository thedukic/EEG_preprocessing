function data = interpolate_epochs(data,elec,bad_elec_epochs,ignore_chans)

assert(size(data,3)==length(bad_elec_epochs));
...
for i = 1:length(bad_elec_epochs)
    if ~isempty(bad_elec_epochs{i}) % && length(bad_elec_epochs{i})<=10
        badchans  = bad_elec_epochs{i};
    ...
    % if  length(bad_elec_epochs{i})>10
    %     warning('A lot of bad channels...');
    % end
end
