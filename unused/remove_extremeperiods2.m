...
fs = EEG(1).srate;
smoothLengthSample = @(smoothLengthMS) round(fs * (smoothLengthMS/1000));
...
for i_blk = 1:NBLK
    ...
    [bh, ah] = butter(2, 0.25/(fs/2), 'high');
    % dataeeg = do_filteringcore(bh,ah,dataeeg,EEG(i_blk).event,fs);
    dataeog = do_filteringcore(bh,ah,dataeog,EEG(i_blk).event,fs);

    [bh, ah] = butter(2, 5/(fs/2), 'high');
    dataeeg = do_filteringcore(bh,ah,dataeeg,EEG(i_blk).event,fs);

    % =====================================================================
    % Absolute threshold to identify absolute amplitude extreme values
    % =====================================================================
    % 350 uV can also capture very strong EMG - is this a good thing?
    % 500 uV can also capture very mild drifts - is this a good thing?
    P = 500;
    extremeMask1 = abs(dataeeg) > P;
    ...
    if any(extremeMask1)
        ...
        P = smoothLengthSample(250);
        extremeMask1 = movmean(extremeMask1, P) > 0;
    ...
    % Set the minimum treshold for blink detection
    ...
    % P = 10;
    P = smoothLengthSample(20);
    data2 = conv(data2, ones(1,P), 'same');
    data2 = movmean(data2', 2*P)';
    ...
    P3 = 15; % X% of elec
    extremeMaskTmp2 = extremeMask2 >= 2*P3;
    extremeMaskTmp2 = extremeMaskTmp2 & maskEOG;

    % Mask
    extremeMask2(extremeMask2 < P3) = 0;
    extremeMask2(extremeMaskTmp2) = 2*P3;

    P = smoothLengthSample(1000);
    extremeMask2 = movmean(extremeMask2, P) > 0;
    ...
    % extremeMaskCombined = extremeMask2;
    ...
    if any(extremeMaskCombined)
        ...
        % Remove:
        % 1. Too short good periods
        % 2. Too short bad periods
        minL = 2*fs;
        extremeMaskFinal = fix_mask(extremeMaskCombined, minL, 1);
        minL = fs/8;
        extremeMaskFinal = fix_mask(extremeMaskFinal, minL, 2);
        ...
        % % EEGTMP2.data(130,:)        = -200 * extremeMask1;
        % % EEGTMP2.data(131,:)        = -200 * extremeMask2;
        % EEGTMP2.data(130,:)        = -800 * extremeMaskFinal;
        % EEGTMP2.data(131:end,:)    = 0 * EEGTMP1.data(131:end,:);
    ...
    % Add for easy visualisation
    % vis_artifacts(EEG(1), EEG(1));
    EEG(i_blk).etc.clean_sample_mask = ~extremeMaskFinal;

    extremeMaskFinalVisual{i_blk} = extremeNoiseEpochs3;

    tmp = EEG(i_blk).ALSUTRECHT.extremeNoise.proportionExcludedForExtremeOutlier;
    if tmp == 0
        fprintf('No extremely bad EEG segments were found.\n');
    elseif tmp < 0.01
        fprintf('Proportion of extremely bad EEG is <1%%.\n');
    else
        fprintf('Proportion of extremely bad EEG: %1.2f\n', tmp);
    end

    fprintf(EEG(i_blk).ALSUTRECHT.subject.fid,'\n---------------------------------------------------------\n');
    fprintf(EEG(i_blk).ALSUTRECHT.subject.fid,'Block %d: Extremely bad epochs\n',i_blk);
    fprintf(EEG(i_blk).ALSUTRECHT.subject.fid,'---------------------------------------------------------\n');
...
% Helper functions
% =========================================================================
function extremeMaskFinal = fix_mask(extremeMaskCombined,minL,flagOpt)
