...
% full R
R = zeros(length(onsets),nbchan,nbchan);
...
for fi = 1:numfrex
    ...
    %%% compute S
    % full S
    S = zeros(length(onsets),nbchan,nbchan);
    for segi = 1:length(onsets)
...
for i = 1:max(freqbands)
    ...
    topoplotIndie(m(:,1),chanlocs,'numcontour',0,'electrodes','off','plotrad',0.5);
