...
% NEW
if ~isempty(events)
    NEOG = length(events.veog);

    fh = figure; hold on;
    dureog = NaN(NEOG,1);
    for i = 1:NEOG
        y = data.veog(events.veog{i}(1):events.veog{i}(2));
        dureog(i) = length(y);
    ...
    for i = 1:NEOG
        noiseMask(events.veog{i}(1):events.veog{i}(2)) = 1;
    end
...
% title(['N = ' num2str(NEOG) ', Treshold = ' num2str(round(treshold))]);
