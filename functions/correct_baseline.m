...
%% check inputs
fprintf('Baseline correction using linear regression...\n');
...
cnt = 0;
for e = 1:size(EEG.event,2) % size(EEG.data,3) % for each event:
    if ismember(EEG.event(e).type,Factor_1_Level_1_Codes)
        cnt = cnt+1;
        confounds(cnt,1) = 0;
    end

    if Factor1_present == 1
        if ismember(EEG.event(e).type,Factor_1_Level_1_Codes{1})
            confounds(cnt,2) = 1;  % code HappyGo as [1 1]
        elseif ismember(EEG.event(e).type,Factor_1_Level_1_Codes{2})
            confounds(cnt,2) = -1; % code HappyNogo as [1 -1]
        end
    end
